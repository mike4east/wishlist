...
<div class="w-50">
    ...
    <form method="POST" action="/dashboard/creaters/{{ $creater->id }}" class="mb-5">
        {{ csrf_field() }}
        <input type="hidden" name="_method" value="PUT">
        <div class="form-inline mt-4 mb-4 row">
            <label for="product-name" class="col-2 d-flex justify-content-start">商品名</label>
            <input type="text" name="name" id="product-name" class="form-control col-8" value="{{ $creater->name }}">
        </div>
        <div class="form-inline mt-4 mb-4 row">
            <label for="product-price" class="col-2 d-flex justify-content-start">価格</label>
            <input type="number" name="price" id="product-price" class="form-control col-8" value="{{ $creater->price }}">
        </div>
        <div class="form-inline mt-4 mb-4 row">
        ...
        <div class="form-inline mt-4 mb-4 row">
            ...
            <textarea name="description" id="product-description" class="form-control col-8" rows="10">{{ $creater->description }}</textarea>
        </div>
    ...
    <div class="d-flex justify-content-end">
        <a href="/dashboard/creaters">商品一覧に戻る</a>
    </div>
</div>
@endsection